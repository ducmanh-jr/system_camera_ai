...
def normalize_plate_prediction(text: str):
    ...
    # GUARD: Only apply when prefix is NOT two digits (province code).
    # Province codes like 71, 47 were being expanded (7→T, 1→T → "TT")
    # which corrupted regular plates (e.g. 71S55600 → TT55600).
    if len(raw) >= 4:
        prefix2 = raw[:2]
        if not (prefix2[0].isdigit() and prefix2[1].isdigit()):
