...
from apps.api.ocr_adapter import VietnamesePlateOCR


ROOT_DIR = Path(__file__).resolve().parents[2]
APP_DIR = ROOT_DIR / "apps" / "api"
WEB_DIR = ROOT_DIR / "web" / "dashboard"
RUNTIME_DIR = ROOT_DIR / "runtime" / "viettraffic"
...
PLATE_PIPELINE_DIR = ROOT_DIR / "giai_doan_tim_bien"
PLATE_OUTPUT_DIR = PLATE_PIPELINE_DIR / "outputs"
WRONG_LANE_DIR = ROOT_DIR / "module_di_sai_lan_duong"
...
def _build_runtime_plan(mode: str, max_frames: int) -> list[dict[str, Any]]:
    ...
    if mode == "plate":
        return [
            ...
            _stage("detecting_plates", plate_seconds, 8, 70, "giai_doan_tim_bien"),
    ...
    if mode == "wrong_lane":
        return [
            ...
            _stage("wrong_lane", wrong_lane_seconds, 10, 96, "module_di_sai_lan_duong"),
            _stage("done", 2, 96, 100, "api"),
        ]
    return [
        _stage("queued", 4, 0, 6, "api"),
        _stage("wrong_lane", wrong_lane_seconds, 6, 75, "module_di_sai_lan_duong"),
...
def _collect_wrong_lane_results(copied_output_dir: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "debug_video": _url_for_runtime_path(copied_output_dir / "debug_wrong_lane.mp4"),
...
def _read_plate_from_capture(image: UploadFile | None, prefix: str) -> tuple[str, dict[str, Any], str]:
    ...
    try:
        ...
        plate_detector_path = ROOT_DIR / "giai_doan_tim_bien" / "models" / "license_plate_detector.pt"
...
def list_parking_images() -> dict[str, Any]:
    images_dir = RUNTIME_DIR / "parking_images"
...
async def ocr_test(image: UploadFile = File(...)) -> dict[str, Any]:
    ...
    try:
        ...
        ocr = VietnamesePlateOCR(use_gpu=False).read_image(ocr_input)
        result["ocr"] = {
            "raw": ocr.raw,
            "plate": ocr.plate,
        ...
        try:
            ocr = VietnamesePlateOCR(use_gpu=False).read_image(save_path)
            result["ocr"] = {
                "raw": ocr.raw,
                "plate": ocr.plate,
                "confidence": round(float(ocr.confidence or 0), 4),
