...
def merge_train2_train1(train2_text: str, train1_text: str) -> str:
    from train_best import normalize_plate_prediction
    
    # Pre-normalize using rule-based normalization to clean up character confusions
    train2_clean = normalize_plate_prediction(train2_text)
    train1_clean = normalize_plate_prediction(train1_text)
    
    train2_norm = normalize_plate_text(train2_clean)
    train1_norm = normalize_plate_text(train1_clean)
    ...
    if is_old_style_plate(train2_norm) and is_old_style_plate(train1_norm) and has_separator(train1_text):
        ...
        # GUARD: Don't add train1 if it's shorter and would lose digits
        # from a valid train2 prediction (fixes ~46% of merge errors).
        len(train_serial) >= 5
        and len(train1_serial) < len(train_serial)
        and plate_validity_score(train2_norm) >= 10
    ):
        candidates.append(train1_norm)
